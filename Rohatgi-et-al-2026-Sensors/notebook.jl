...
md"""
Modern seismic acquisition is increasingly built around dense networks of point-receiver sensors, including single component geophones, autonomous nodal sys-tems, and distributed acoustic sensing (DAS) arrays, that sample the recorded wavefield with far greater spatial density than legacy source and receiver arrays used in conven-tional seismic acquisition [1,2].

Seismic wavefields recorded by these sensors are inevitably distorted by near-surface heterogeneity and small-scale elastic contrasts. These perturbations scatter energy and introduce complex, frequency-dependent distortions of amplitude and phase [3]. In land seismic processing, such effects have traditionally been mitigated through the extensive use of source and receiver arrays during acquisition [4], followed by surface-consistent processing methods. Surface-consistent deconvolution and residual statics model the data using operators assigned separately to sources and receivers, implicitly assuming that near-surface distortions repeat in a predictable manner across traces [5,6]. Randomness is accommodated only in a limited sense, typically through trace-dependent static time shifts parameterized by one operator per source and one per receiver. These methods were deliberately designed to limit the number of degrees of freedom and overdetermine the estimation problem, enabling robust first-order corrections for statics and coupling, but without being grounded in a physical model of wave propagation through heterogeneous media.
Cary and Nagarajappa [7,8] explicitly demonstrated that surface-consistent decon-volution can leave substantial residual phase errors and proposed additional sur-face-consistent phase corrections coupled with residual statics estimation. However, the underlying assumption remained that phase corrections are frequency-independent and surface-consistent. As a result, these approaches remain fundamentally unable to address the trace-to-trace, frequency-dependent phase variability that is abundant in modern single-sensor data. Several extensions have sought to relax strict surface consistency by introducing non-surface-consistent, time-varying statics that allow trace-dependent perturbations [9]. While these methods come closest to acknowledging the increased complexity of near-surface effects within conventional processing, they remain funda-mentally constrained. By construction, they address only travel-time corrections and do not account for localized, frequency-dependent phase perturbations arising from small-scale scattering. Consequently, the available degrees of freedom remain insufficient to represent waveform distortions that vary continuously along the trace and differ for each source–receiver pair.

In practice, wave propagation through heterogeneous near-surface media is not surface consistent. Small-scale heterogeneity induces forward scattering and interference that generate non–surface-consistent, trace-specific phase perturbations [10]. These per-turbations are not purely kinematic and cannot be corrected by time shifts alone. Instead, they modify the phase locally in time and frequency, even after advanced statics and deconvolution have been applied. As a result, residual phase disorder persists throughout prestack gathers, revealing a fundamental gap between classical processing assumptions and the physics of wave propagation in heterogeneous media [11,12].

Ensemble-based processing methods implicitly acknowledge the importance of phase coherence by exploiting redundancy in multi-channel seismic data. Historically, this redundancy was realized directly in acquisition through the use of source and re-ceiver arrays, where local stacking in the time domain improved phase coherence but simultaneously conditioned phase and amplitude, a limitation perceived as detrimental to amplitude fidelity and high-frequency preservation [13]. In seismology, phase-weighted stacks were introduced to explicitly prioritize phase coherence for signal detection, re-flecting the recognized outsized importance of recovering correct phase from mul-ti-channel recordings [14,15]. Phase-estimation and enhancement methods based on local attributes and blind deconvolution were later proposed to stabilize phase behavior [16–18]. More recently, with the widespread adoption of point-receiver acquisition, ensem-ble-based phase conditioning has been reintroduced into reflection seismic processing through seismic time-frequency masking, where phase manipulation is decoupled from amplitude effects [19]. While these methods effectively reduce incoherent phase locally, they treat phase variability primarily as a nuisance to be suppressed and do not explicitly analyze phase as a statistical quantity or quantify the underlying phase distributions that govern coherence loss.

Despite decades of advances in noise attenuation and imaging, and long-standing recognition of the critical role of phase coherence, seismic processing still lacks a routine, quantitative, and objective measure of phase reliability. In practice, phase quality is assessed indirectly through visual inspection, stack response, or amplitude-based attributes, relying on ad-hoc heuristics and human judgment rather than a standardized metric that isolates phase behavior or captures its frequency-dependent variability. This limitation is most evident in land environments with strong near-surface heterogeneity, where pronounced phase variability persists despite careful conventional processing [12]. The problem becomes acute in desert environments such as the Middle East, where near-surface heterogeneity is strong [20,21], and reaches its extreme in highly scattering media such as basalts and crystalline rocks, where seismic imaging is often restricted to very low frequencies [22].

Recent work has reframed near-surface scattering as a multiplicative process that randomizes seismic phase, giving rise to speckle-like behavior in seismic wavefields [12,20,22]. Analogous phenomena are well documented in optics and acoustics, where volumetric scatterers produce frequency-dependent phase fluctuations and coherence loss that are best described statistically [23–25]. In seismic data, forward scattering on me-ter-scale heterogeneity generates multiple near-ballistic arrivals whose interference leads to rapid, trace-to-trace phase variations. These phase perturbations often dominate waveform degradation and increase with frequency, driving the wavefield toward a speckle regime in which individual traces appear chaotic, yet ensemble statistics remain stable and physically meaningful.
Modern land seismic acquisition is characterized by dense spatial sampling with point receivers rather than source-receiver arrays. While this increases spatial resolution and preserves high-frequency information, it also makes small-scale near-surface het-erogeneity and trace-to-trace phase variability far more apparent within localized, densely sampled ensembles. Because single-sensor records carry lower per-trace sig-nal-to-noise ratio than legacy array recording, evaluating phase variability directly at the sensor level becomes essential rather than optional for acquisition systems built on point receivers. In such settings, propagation through heterogeneous near-surface media renders the observed seismic phase effectively random from trace to trace.

This study adopts an explicitly stochastic viewpoint. For any single, fixed source–receiver ray path, the response is indeed fully deterministic and repeatable: firing the same deterministic source through the same near-surface volume produces the same waveform. However, near-surface heterogeneity varies spatially at a scale finer than receiver spacing and is not resolved or known in practice. Each trace in an ensemble corresponds to a different source-receiver ray path, which threads a different, effectively uncharacterized configuration of near-surface scatterers. As a result, the phase distortion accumulated along each path differs from one trace to the next, even though every in-dividual path is governed by fully deterministic wave physics.
Importantly, seismic phase is inherently circular rather than linear and must be treated accordingly. We introduce an ensemble-based framework that models seismic phase as a circular random variable and applies circular statistics to quantify phase variability within prestack gathers. While Rohatgi et al. [22] introduced circular statistics as a tool for characterizing seismic phases, that work does not provide an in-depth treatment of the underlying algorithm, nor does it include validation, verification, or justification of the parameters involved. The present study addresses these gaps directly, providing a detailed algorithmic formulation, synthetic validation of the attribute’s re-covery accuracy, and a systematic analysis of estimator stability with respect to ensemble size.

We define circular variance as phase-variance attribute that summarizes frequen-cy-dependent phase disorder. By shifting from single-trace phase estimates to ensemble phase distributions, the proposed framework provides a physically grounded measure of scattering-induced phase noise and establishes a transparent link among near-surface heterogeneity, phase disorder, and seismic coherence.
...
begin
    ...
    plot!(p7a, SNR_value_processed, lw=3, label="Processed",
          color=:black, linestyle=:dash, title="SNR vs Offset")
...
md"""
Treating seismic phase as a circular variable reframes phase variability from a difficult, trace-level attribute into a measurable ensemble property that can be evaluated systematically across frequency and offset. Circular variance, here called phase variance V(ω), operates directly on wrapped phases and avoids the ambiguities of phase un-wrapping. Low V(ω) corresponds to tightly clustered phases and coherent phase behavior, whereas high V(ω) indicates strong phase dispersion, which may arise from noise, scattering, interfering events, or other wavefield complexity. In addition, the circular mean phase provides a useful estimate of the underlying signal phase within an en-semble. Although not the focus of this study, this estimate may support additional quality control of phase trends and serve as a key ingredient in phase-enhancement approaches such as seismic time-frequency masking [18] and phase-based filtering.

Synthetic tests confirm that V(ω) reliably recovers imposed phase variability as a function of frequency and offset. Application to field data shows that conventional time-domain processing primarily reduces phase variance at low frequencies and mid-to-far offsets. At higher frequencies, phase variance often remains elevated, even when amplitude spectra suggest apparent bandwidth extension. This discrepancy highlights a key limitation of amplitude-based diagnostics and motivates defining an effective bandwidth in terms of phase coherence rather than amplitude alone.
Phase variance is fundamentally a measure of trace-to-trace phase coherence for reflection event within a selected local ensemble. It quantifies phase variability at a given frequency but does not, by itself, identify the physical origin of the coherent energy. Low phase variance therefore indicates the presence of a coherent arrival consistent with the geometry of the analysis window. When the window is aligned with the moveout of a target reflection, low phase variance indicates coherent reflection energy; if the same analysis were instead aligned with coherent ground roll, guided waves, or another or-ganized wave mode, those arrivals could likewise exhibit low phase variance.

In complex data, the analysis window should therefore be centered on the reflection event of interest and, where appropriate, follow its local moveout or dip. This minimizes contributions from neighboring or crossing events. When several arrivals or noise mechanisms overlap within the same window, the phase distribution may become asymmetric or multimodal. In such cases, phase variance remains a useful scalar measure of the overall phase dispersion, but it does not resolve the individual phase populations or identify their separate physical origins.
Under low-SNR conditions, the ensemble-based formulation benefits from the density of modern point-receiver acquisition: dense single-sensor deployments provide a sufficiently large number of closely spaced traces to form statistically stable ensembles, even within a compact spatial neighborhood. By pooling multiple traces at similar offset, phase distortions are quantified collectively. This allows phase consistency to be char-acterized in a frequency-dependent manner even when individual traces have low SNR. At frequencies where spectral amplitudes approach the noise floor, however, phase variance should be interpreted together with amplitude or SNR information.

Phase variance maps and spectra provide a direct way to identify where phase coherence improves or deteriorates after processing. For a selected application-dependent threshold on V(ω), practitioners can delineate a phase-coherent frequency range and track how this range changes with offset or processing stage. A similar objective-based ap-proach has been demonstrated for conventional seismic SNR, where minimum da-ta-quality requirements were related to specific structural and amplitude-dependent interpretation objectives. Similarly, application-specific phase-variance thresholds could be established by relating V(ω) to the performance requirements of the interpretation task. Such information may be useful for phase-sensitive workflows including AVO analysis, full-waveform inversion, and migration. Unlike visual inspection or broadband SNR metrics, V(ω) isolates phase behavior and reveals frequency-dependent limitations that would otherwise remain hidden. The statistical stability of V(ω) and its dependence on ensemble size are examined in Appendix A.
...
md"""
Here, we develop, validate, and demonstrate phase variance as a practical fre-quency- and offset-dependent framework for diagnosing phase variability in seismic sensor-recorded data. By treating phase as a circular variable rather than a trace-level artifact, phase variability becomes a measurable ensemble property that can be quan-tified, tracked, and used to inform acquisition and processing decisions rather than re-lying on visual inspection alone. Circular variance is not merely another quality-control attribute but a physically grounded descriptor of trace-to-trace phase coherence that characterizes phase behavior across frequency and offset.
Our synthetic and field results show that amplitude spectra derived from the seismic recordings can substantially overestimate usable bandwidth, since standard processing often amplifies high-frequency amplitudes while leaving phase behavior noise-dominated. Phase variance exposes this limitation, supporting the definition of an effective bandwidth based on phase coherence rather than spectral amplitude alone. For seismic acquisition, where signal fidelity at higher frequencies is increasingly demanded, this distinction has direct implications for how recording bandwidth and sensor per-formance are specified and evaluated. This motivates a broader view of how seismic data quality is assessed for phase-sensitive applications.

We propose phase variance as a complementary prestack diagnostic for seismic data, to be evaluated alongside amplitude spectra and SNR at acquisition and processing stages. Tracking V(ω) through a processing sequence provides an objective way to identify where phase integrity improves or deteriorates, to guide parameter selection, and to prevent the propagation of phase distortions that would otherwise remain in-visible. Importantly, this diagnostic view naturally enables phase-conditioning and masking strategies driven by explicit objectives, such as achieving a prescribed level of phase fidelity for imaging or inversion, rather than by visual QC or amplitude-based criteria, offering a route toward more principled acquisition design and processing workflows. Its use for downstream applications such as AVO, migration, and inversion should be calibrated against application-specific performance criteria.

The field demonstration presented here is based on a single dataset, representing one geological setting, acquisition geometry, sensor type, and noise environment. While the synthetic validation confirms that phase variance behaves as expected under controlled, known conditions, the generality of the field-data findings, including the specific fre-quency range identified as phase-coherent, should not be assumed to extend automati-cally to other settings. Testing phase variance across datasets with different geology, acquisition geometries, sensor types (e.g., 3C, DAS, nodal), and noise conditions is an important next step toward establishing it as a general-purpose diagnostic.

More broadly, this work develops phase variance into a practical and extensible tool for seismic phase analysis, with direct relevance to sensor array design and acquisition QC, and potential relevance to phase-sensitive imaging and inversion. As seismic ac-quisition moves toward denser sensor sampling, higher-frequency recording, and greater sensitivity to subtle phase effects, we anticipate that phase-based diagnostics and con-ditioning methods grounded in circular statistics will play an increasingly important role in assessing the reliability of seismic data throughout the acquisition-to-inversion workflow.
...
md"""
[1] Abukrat, Y.; Sinitsyn, P.; Reshef, M.; Lellouch, A. Directional sources and distributed acoustic sensing deployments for near-surface characterization. *Geophysics* **2023**, *88*, WC121–WC131. https://doi.org/10.1190/geo2023-0067.1.

[2] Busanello, G.; Bachrach, R.; Sayed, A.; Soliman, B. Surface distributed acoustic sensing for high-resolution near-surface characterization: Results from a 3D onshore field experiment. *Geophysics* **2024**, *89*, KS39–KS51. https://doi.org/10.1190/geo2023-0084.1.

[3] Sato, H.; Fehler, M.C.; Maeda, T. *Seismic Wave Propagation and Scattering in the Heterogeneous Earth*, 2nd ed.; Springer: New York, NY, USA, 2012.

[4] Meunier, J. *Seismic Acquisition from Yesterday to Tomorrow*; Distinguished Instructor Series, No. 14; SEG and EAGE: Tulsa, OK, USA, 2011.

[5] Taner, M.T.; Koehler, F.; Alhilali, K.A. Estimation and correction of near-surface time anomalies. *Geophysics* **1974**, *39*, 441–463.

[6] Taner, M.T.; Koehler, F. Surface-consistent corrections. *Geophysics* **1981**, *46*, 17–22.

[7] Cary, P.; Nagarajappa, N. Robust surface-consistent residual statics and phase correction—Part 1. In *Proceedings of the GeoConvention 2014, FOCUS: Adapt, Refine, Sustain*, Calgary, AB, Canada, 12–16 May 2014.

[8] Cary, P.; Nagarajappa, N. Robust surface-consistent residual statics and phase correction—Part 2. In *Proceedings of the GeoConvention 2014, FOCUS: Adapt, Refine, Sustain*, Calgary, AB, Canada, 12–16 May 2014.

[9] Reilly, J.M.; Shatilo, A.P.; Shevchek, Z.J. The case for separate sensor processing: Meeting the imaging challenge in a producing carbonate field in the Middle East. *Lead. Edge* **2010**, *29*, 1240–1249.

[10] Bakulin, A.; Neklyudov, D.; Silvestrov, I. Multiplicative seismic noise caused by small-scale near-surface scattering and its transformation during stacking. *Geophysics* **2022**, *87*, V419–V435.

[11] Stork, C. How does the thin near surface of the Earth produce 10–100 times more noise on land seismic data than on marine data? *First Break* **2020**, *38*, 67–75.

[12] Bakulin, A.; Silvestrov, I.; Dmitriev, I.; Neklyudov, D.; Protasov, M.; Gadylshin, K.; Dolgov, V. Nonlinear beamforming for enhancement of 3D prestack land seismic data. *Geophysics* **2020**, *85*, V283–V296.

[13] Newman, P.; Mahoney, J.T. Patterns—With a pinch of salt. *Geophys. Prospect.* **1973**, *21*, 197–219.

[14] Schimmel, M.; Paulssen, H. Noise reduction and detection of weak, coherent signals through phase-weighted stacks. *Geophys. J. Int.* **1997**, *130*, 497–505.

[15] Schimmel, M.; Gallart, J. Frequency-dependent phase coherence for noise suppression in seismic array data. *J. Geophys. Res. Solid Earth* **2007**, *112*, B04303.

[16] van der Baan, M. Time-varying wavelet estimation and deconvolution by kurtosis maximization. *Geophysics* **2008**, *73*, V11–V18.

[17] Fomel, S.; van der Baan, M. Local skewness attribute as a seismic phase detector. *Interpretation* **2014**, *2*, SA49–SA56.

[18] Holt, R.; Lubrano, A. Stabilizing the phase of onshore 3D seismic data. *Geophysics* **2020**, *85*, V473–V479.

[19] Bakulin, A.; Neklyudov, D.; Silvestrov, I. Seismic time-frequency masking for suppression of seismic speckle noise. *Geophysics* **2023**, *88*, V371–V385.

[20] Bakulin, A.; Neklyudov, D.; Silvestrov, I. The impact of receiver arrays on suppressing seismic speckle scattering noise caused by meter-scale near-surface heterogeneity. *Geophysics* **2024**, *89*, V551–V565.

[21] Ziolkowski, A.; Hanssen, P.; Gatliff, R.; Li, X.-Y.; Jakubowicz, H. Use of low frequencies for sub-basalt imaging. *Geophys. Prospect.* **2003**, *51*, 169–182.

[22] Rohatgi, A.; Bakulin, A.; Fomel, S. Data-driven analysis of seismic phase using circular statistics. *Lead. Edge* **2025**, *44*, 683–691.

[23] Goodman, J.W. *Speckle Phenomena in Optics: Theory and Applications*; Roberts & Company: Englewood, CO, USA, 2007.

[24] Abbott, J.G.; Thurstone, T.H. Acoustic speckle: Theory and experimental analysis. *Ultrason. Imaging* **1979**, *1*, 303–324.

[25] Gosselin, J.M.; Audet, P.; Estève, C.; Schaeffer, A.J. Probabilistic inversion of circular phase spectra: Application to two-station phase-velocity dispersion estimation in western Canada. *Geophys. J. Int.* **2023**, *233*, 1387–1398. https://doi.org/10.1093/gji/ggac506.

[26] Zebker, H.A.; Lu, Y. Phase unwrapping algorithms for radar interferometry: A systematic comparison. *J. Opt. Soc. Am. A* **1998**, *15*, 586–600.

[27] Feigl, K.L.; Smith, R.M.; Behn, F.D.; Sheehan, A.F. A method for modeling radar interferograms without phase unwrapping. *Geophys. J. Int.* **2009**, *176*, 491–497.

[28] Shatilo, A.P. Seismic phase unwrapping: Methods, results, problems. *Geophys. Prospect.* **1992**, *40*, 211–225.

[29] Mohammad, H.H.; Satari, S.Z.; Yusoff, W.N.S.W. Review on circular–linear regression models. *J. Phys. Conf. Ser.* **2021**, *1988*, 012108. https://doi.org/10.1088/1742-6596/1988/1/012108.

[30] Mardia, K.V.; Jupp, P.E. *Directional Statistics*; John Wiley & Sons: Chichester, UK, 2000.

[31] von Mises, R. Über die “Ganzzahligkeit” der Atomgewichte und verwandte Fragen. *Phys. Z.* **1918**, *19*, 490–500.

[32] Jiang, Y.; Hu, J.; Li, Z.; Liu, Q. Bayesian inversion of wrapped satellite interferometric phase. *Geophys. J. Int.* **2019**, *219*, 1500–1518.

[33] Takamichi, S.; Kobayashi, K.; Toda, T.; Saruwatari, H. Phase reconstruction from amplitude spectrograms based on a von Mises-distribution deep neural network. In *Proceedings of the Interspeech 2018*, Hyderabad, India, 2–6 September 2018; pp. 2027–2031.

[34] Nugraha, A.A.; Kobayashi, K.; Toda, T.; Sakti, S.; Nakamura, S. A deep generative model of speech complex spectrograms. *IEEE/ACM Trans. Audio Speech Lang. Process.* **2019**, *27*, 63–76.

[35] Gosselin, J.M. Probabilistic inversion of circular phase spectra: Application to two-station phase-velocity dispersion estimation. *Geophys. J. Int.* **2022**, *229*, 1492–1517.

[36] Fisher, N.I. *Statistical Analysis of Circular Data*; Cambridge University Press: Cambridge, UK, 1993.

[37] Rohatgi, A.; Bakulin, A.; Fomel, S. Amplitude-invariant phase masking for coherence recovery in scattered wavefields. *JASA Express Lett.* **2026**, *6*, 074801.
...
md"""
Circular variance is a statistical quantity estimated from a finite ensemble of phase samples. As such, the variance itself is a random variable whose stability depends on the number of traces included in the ensemble. Importantly, small ensemble sizes do not render phase-variance estimates uninterpretable; rather, they increase estimator varia-bility and make interpretation more sensitive to local fluctuations. Interpretable phase-variance estimates therefore require an ensemble size appropriate to the objective of the analysis. To assess the effect of ensemble size, we computed circular variance using densely overlapping sliding windows (stride = 1 trace) containing 10, 100, and 1000 traces (Figure A1). For small ensembles, the estimated variance exhibits strong local fluctuations due to limited sampling, revealing fine-scale detail but with increased statistical scatter. As the number of traces increases, these fluctuations are progressively suppressed, and the estimated variance converges toward a smooth and stable trend that reflects the underlying phase dispersion rather than estimator noise. Importantly, the large-scale trend in phase variability is preserved across all window sizes, while increasing the ensemble size improves statistical stability. To quantify this dispersion, we compute the RMS deviation between the estimated and true phase variance for each window size, yielding RMSE values of 0.176, 0.055, and 0.012 for N = 10, 100, and 1000 traces, re-spectively. This provides a direct, data-driven estimate of uncertainty as a function of ensemble size, complementing the qualitative trend shown in Figure A1.
